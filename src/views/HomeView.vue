...
<template>
  ...
  <section class="w-full pt-4" v-else>
    ...
    <div class="flex gap-4 sm:gap-6 flex-wrap justify-center items-center w-full pl-4 sm:pl-4">
      <SpotCard v-for="show in filteredShows" :key="show.id" :posterImg="show.images[1].url"
        :imageWidth="show.images[1].width" :imageHight="show.images[1].height" :posterName="show.name"
